...
Na het trainen van het model met de best mogelijke hyperparameters, vergelijk ik de trein- en testgegevens met behulp van het classificatierapport van SKLearn. In eerste instantie kijk ik naar de nauwkeurigheid, om te kijken welk percentage van alle gevallen het model juist voorspelde: in dit geval is de nauwkeurigheid 78%, wat een prima score is. Deze nauwkeurigheid geldt voor de trainingsgegevens. Voorafgaand aan de training van het model zijn de gegevens gesplitst in een trainingsset, en een testset, om te zien hoe het model presteert op ongeziene gegevens(cross validation). Op de testgegevens geeft het model een nauwkeurigheid van : 74%. Het ziet er naar uit dat het model niet aan het overfitten, of underfitten is, doordat ze niet veel van elkaar verschillen.  Voor de zekerheid heb ik een ROC curve aan de notebook toegevoegd, om te evalueren of de test set een goede true positive/false positive ratio heeft, en de ROC curve ziet er gezond uit.
